...
export default function JoinForm() {
  ...
  const saveJoin = (e) => {
    ...
    if (userPswd.value !== userPswd2.value) {
      fn.alertMessage('비밀번호와 비밀번호확인은 같아야 합니다.', '회원가입').then();
      document.querySelector('#userPswd2').focus();
      return false;
    }

    fn.confirmMessage('회원 가입 하시겠습니까?', '회원가입').then((isOk) => {
      if (isOk) {
        fn.requestApi(
          'post',
          adminUrl.JOIN,
          ...
          if (res.success) {
            fn.alertMessage('정상 처리 되었습니다', '회원가입').then((r) => history.push('sign-in'));
          }
        });
      }
    });
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">가입을 위한 정보를 입력하세요.</p>
        </div>
        <form id="frmJoin" className="mt-8 space-y-6">
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="flex space-x-2">
                ...
                <button onClick={idCheck} type="button" className="bt-gray flex-initial w-1/3 ">
                  중복확인
                </button>
              </div>
              <span id="spanIdValid" className="input-msg-red">
                아이디를 입력해 주세요(영문 또는 숫자 6자 이상 20자 미만)
              </span>
            </div>
            <div>
            ...
            <div>
              <label htmlFor="userMbl" className="input-label">
                휴대폰 번호
              </label>
              <div className="flex space-x-2">
                <input
                  ...
                  name="userMbl"
                  type="text"
                  autoComplete="userMbl"
                  required
                  className="input-basic flex-initial w-full"
                  placeholder="'-'구분없이 입력"
                  {...userMbl}
                />
                <button onClick={moblieCheck} type="button" className="bt-gray flex-initial w-1/3 ">
                  인증번호 전송
                </button>
              </div>
              <span id="spanTelValid" className="input-msg-red">
                규칙에 맞는 휴대폰 번호를 입력해 주세요.
              </span>
            </div>
            <div>
              <label htmlFor="mobileAuth" className="input-label">
                인증번호
              </label>
              <input
                id="mobileAuth"
                ...
                {...mobileAuth}
              />
              <span id="" className="input-msg-blue">
                번호 인증 완료
              </span>
              <span id="" className="input-msg-red">
                인증번호가 틀렸습니다. 다시 확인해주세요.
              </span>
            </div>

            <div>
              <label htmlFor="userPswd" className="input-label">
                비밀번호
              </label>
              <input
                id="userPswd"
                name="userPswd"
                type="password"
                autoComplete="userPswd"
                required
                className="input-basic w-full"
                placeholder="비밀번호"
                {...userPswd}
              />
              <span id="spanPasswordValid" className="input-msg-red">
                8자 이상, 숫자와 특수문자 포함을 권장합니다.
              </span>
            </div>
            <div>
              <label htmlFor="userPswd2" className="input-label">
                비밀번호 확인
              </label>
              <input
                id="userPswd2"
            ...
            <div>
              ...
              <input id="memo" name="memo" type="text" className="input-basic w-full" placeholder="하고싶은 말" />
            </div>
          </div>
          ...
          <div>
            <button type="button" onClick={saveJoin} className="bt-indigo group w-full flex ">
              회원가입하기
            </button>
          </div>
        </form>
                ...
                <div className="mt-3 sm:mt-0 sm:ml-4ms-start">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500 overflow-auto h-40">
                      ...
                      제 1 조 (목적)
                      <br />
                      본 약관은 주식회사 000(이하 ”회사”라 합니다)에서 제공하는 인터넷 관련 서비스(접속 가능한 유•무선
                      단말기의 종류와 관계없이 회사가 제공하는 이용 가능한 모든 서비스를 의미하며, 이하 “서비스”라
                      합니다)를 이용함에 있어 회사와 회원의 권리와 의무, 책임사항을 규정함을 그 목적으로 합니다.
                      <br />
                      <br />
                      제 2 조 (용어의 정의)본 약관에서 사용하는 용어의 정의는 다음과 같습니다.
                      <br />
                      1. “사이트”란 회사가 재화 또는 서비스(이하 “상품 등”이라 합니다)를 회원에게 제공하기 위하여 컴퓨터
                      등 정보통신설비를 이용하여 상품 등을 거래할 수 있도록 설정한 가상의 영업장을 말하며 회사가 모바일
                      환경에서 서비스하는 모바일 웹과 앱을 포함합니다.
                      <br />
                      2. “회원”이라 함은 사이트에서 정한 소정의 절차를 거쳐 회원가입을 한 자로서, 약관에 따라 회사가
                      제공하는 서비스를 이용할 수 있는 자를 말합니다.
                      <br />
                      3. “아이디(ID)”라 함은 회원의 식별과 서비스의 이용을 위하여 회원이 설정하고 회사가 승인하여 등록된
                      전자우편주소 또는 소셜 서비스 연동을 통해 수집된 전자우편주소를 말합니다.
                      <br />
                      4. “메일 인증”이라 함은 회원이 서비스의 이용을 위하여 제출한 인증번호를 통해 이메일의 진위여부를
                      확인하는 것을 말합니다.
                      <br />
                      5. “비밀번호(Password)”라 함은 회원의 동일성 확인과 회원의 권익 및 비밀보호를 위하여 회원 스스로가
                      설정하여 사이트에 등록한 문자와 숫자의 조합을 말합니다.
                      <br />
                      6. “포인트”라 함은 상품 등을 구매하거나 사이트가 제공하는 서비스를 이용할 때 현금처럼 사용할 수
                      있는 사이트 전용의 사이버 화폐를 말합니다.
                      <br />
                      7. “쿠폰”이라 함은 상품 등을 구매하거나 사이트가 제공하는 서비스를 이용할 때 표시된 금액 또는
                      비율만큼 할인을 받을 수 있는 쿠폰을 말합니다.
                      <br />
