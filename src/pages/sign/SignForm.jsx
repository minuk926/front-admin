...
export default function SignForm() {
  ...
  function login(e) {
    ...
    if (!userId.value || !regExpId.test(userId.value)) {
      fn.alertMessage('id를 확인해 주세요.', '로그인').then();
    ...
    if (!userPswd.value || !regExpPw.test(userPswd.value)) {
      fn.alertMessage('비밀번호를 확인해 주세요.', '로그인').then();
      document.querySelector('#userPswd').focus();
      return false;
    }

    fn.requestApi('post', adminUrl.LOGIN, {
      userId: userId.value,
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        <div>
          <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">아이디 로그인</h2>
          <p className="mt-2 text-center text-sm text-gray-600">아이디와 패스워드를 입력하세요.</p>
        </div>
        <form id="frmLogin" className="mt-8 space-y-6">
          ...
          <div className="rounded-md shadow-sm -space-y-px">
            <div>
              ...
              <input id="userId" name="userId" {...userId} required className="input-login w-full rounded-t-md" placeholder="아이디" />
            </div>
            <div>
              <label htmlFor="userPswd" className="sr-only">
                패스워드
              </label>
              <input id="userPswd" name="userPswd" type="password" {...userPswd} required className="input-login w-full rounded-b-md" placeholder="패스워드" />
            </div>
          </div>
          ...
          <div>
            <button onClick={login} className="bt-indigo group w-full flex">
        ...
        <div>
          <p className="mt-2 text-center text-sm text-gray-600">
            아직 계정이 없으신가요?{' '}
            <a href="#" onClick={goJoin} className="font-bold text-indigo-600 hover:text-indigo-500 hover:underline ">
              회원가입하기
            </a>
          </p>
        </div>
